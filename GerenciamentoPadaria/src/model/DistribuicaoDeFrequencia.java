...
public class DistribuicaoDeFrequencia {
    ...
    private BigDecimal totalFrequenciaRelativa;
    private BigDecimal totalFrequencia;
    private BigDecimal totalFrequenciaRelativaPorcentagem;
    ...
    public void efetuarCalculos(List<BigDecimal> lista) {
        ...
        if (!lista.isEmpty()) {
            ...
            if (amplitudeIntervalos.compareTo(BigDecimal.ONE) > 0) {
                amplitudeIntervalos = new BigDecimal(String.valueOf(amplitudeIntervalos.intValue() + 1));
                // Arredondamento
                // dos
                // intervalos,
                // versão
                ...
                amplitudeIntervalos = amplitudeIntervalos.round(new MathContext(2, RoundingMode.CEILING)); // Se o
                // numero
                // for menor
                // que 1 se
                // limitara
            ...
            if (testeIntervalo.compareTo(max) == 0) {
                amplitudeIntervalos = amplitudeIntervalos.add(new BigDecimal("0.01"));
                // Compara se mesmo após o
                // arredondamento para dezena
                // mais proxima o valor bate
                // exatamente
            }   // com o teto da ultima classe da tabela (O que acaba inutilizando as variaveis
            // neste teto)
            ...
            for (int smtr = 0; smtr < classes.intValue(); smtr++) {
                ...
                frequenciaRelativa = frequenciaAbsoluta.divide(new BigDecimal(String.valueOf(quantiVariaveis)),
                        new MathContext(3));
                ...
                pontoMedio = pontoMedio.add(limiteInferior).add(limiteSuperior).divide(BigDecimal.TWO,
                        new MathContext(5));
