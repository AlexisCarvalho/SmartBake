	...
	public void efetuarCalculos(List<BigDecimal> lista) {
		...
		if (!lista.isEmpty()) {
			...
			if (amplitudeIntervalos.compareTo(BigDecimal.ONE) > 0) {
				amplitudeIntervalos = new BigDecimal(String.valueOf(amplitudeIntervalos.intValue() + 1)); // Arredondamento
																											// dos
																											// intervalos,
																											// versão
																											// inteira
			...
			if (testeIntervalo.compareTo(max) == 0) {
				amplitudeIntervalos = amplitudeIntervalos.add(new BigDecimal("0.01")); // Compara se mesmo após o
